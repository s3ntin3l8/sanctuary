...
- **Issuers of certifying evidence are `third_party`.** Banks issuing deposit certificates / Vermögensnachweise / Kontoauszüge (ICBC, Deutsche Bank, Sparkasse, …), notaries (Notar, Notariat) issuing beglaubigte Urkunden, sworn translators / interpreter certifying authorities (Übersetzer-Beglaubigung, beglaubigte Übersetzung), foreign government registrars (Standesamt, Hukou-Behörde / 公安局, consulates), Grundbuchamt extracts, Handelsregister extracts, certified medical opinions (Atteste from non-court-appointed physicians) — `originator_type` is the **credentialing institution**, never the party whose case the evidence supports. **Do not flip `originator_type` to `own`/`opposing` based on which side benefits.** A bank deposit certificate submitted by the opposing party to prove their financial capacity is still authored by the bank → `third_party`.
