...
from app.config import OLLAMA_BASE_URL, OLLAMA_EMBED_MODEL, SessionLocal
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                ...
                json={"model": OLLAMA_EMBED_MODEL, "prompt": content_snippet},
            )
            ...
            if embedding:
                # sqlite-vec needs the array serialized as JSON or raw bytes
                # depending on usage. We store it as a JSON string for now,
                # and parse it in queries, or convert it to bytes.
                # Actually, `sqlite-vec` requires binary (f32).
                # The `content_embedding` column in `Document` is `Text`.
...
async def check_embedding_status() -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            status["embedding_model"] = any(OLLAMA_EMBED_MODEL in m for m in models)
    except Exception as e:
        status["error"] = str(e)
