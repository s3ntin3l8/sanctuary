...
def test_costs_crud_flow(app_client, sample_case):
    # 1. Create a cost
    resp = app_client.post("/costs", data={
    ...
    cost_id = 1 
    resp = app_client.post(f"/costs/{cost_id}/update-field", data={
        "field": "title",
        "value": "Updated Fee Title"
    })
    assert resp.status_code == 200
...
def test_promote_cost_delta(app_client, db_session, sample_case):
    ...
    cost = db_session.query(LegalCost).filter(LegalCost.source_document_id == doc.id).first()
